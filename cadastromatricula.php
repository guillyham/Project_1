...
if (empty($termo)):
  ...
  // Executa uma consulta baseada no termo de pesquisa passado como parâmetro
  $conexao = conexao::getInstance();
  $sql_aluno = 'SELECT id, nome FROM tab_aluno WHERE nome LIKE :nome';
  $stm_aluno = $conexao->prepare($sql_aluno);
  $stm_aluno->bindValue(':nome', $termo.'%');
  $stm_aluno->execute();
  $aluno = $stm_aluno->fetchAll(PDO::FETCH_OBJ);

  $conexao = conexao::getInstance();
  $sql_turma = 'SELECT id, turno, semestre, curso, turma FROM tab_turma WHERE curso LIKE :id';
  $stm_turma = $conexao->prepare($sql_turma);
  $stm_turma->bindValue(':id', $termo.'%');
  $stm_turma->execute();
  $turma_c = $stm_turma->fetchAll(PDO::FETCH_OBJ);
?>
        ...
        <input type="hidden" name="acao" value="incluir" />
         ...
         <div class="form-group">
            ...
            <select class="form-control" name="aluno">
              <?php if ($aluno) {
                foreach($aluno as $inda => $valorAluno) {
               ?>
                <option value="<?php echo $valorAluno->id ?>"><?php echo $valorAluno->nome ?></option>
              <?php } } ?>
            </select>
          </div>

          <div class="form-group">
              <label for="turma">Curso, Semestre, Turno, Turma </label>
            <select class="form-control" name="turma">
              <?php if ($turma_c) {
                foreach($turma_c as $inds => $valorTurma) {
               ?>
                <option value="<?php echo $valorTurma->id ?> "> 
                               <?php echo $valorTurma->curso ?> |                               
                               <?php echo $valorTurma->semestre ?> | 
                               <?php echo $valorTurma->turno ?> | 
                               <?php echo $valorTurma->turma ?>
                                 </option>
              <?php } } ?>
            </select>
          </div>
